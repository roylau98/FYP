...
from ESP32_CSI_Visualiser.utilities.classes import Xaxis
...
class Mpl(qtw.QGroupBox):
    ...
    def heatmap_plot(self, CSI_DATA, MAC, csi_type):
        ...
        self.ax = self.canvas.figure.add_subplot(111)
        self.canvas.draw()
        sns.heatmap(ax=self.ax, data=CSI_DATA, cbar_kws={'label': f'CSI {csi_type}'})
        self.ax.set_title(f"Heatmap of {MAC}")
